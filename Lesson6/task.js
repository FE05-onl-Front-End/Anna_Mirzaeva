...
arr.forEach((key) => {
    ...
    {
        if (typeof arr[index] === 'string' && typeof key === 'string') {
            let lowKey = key.toLowerCase();
            let lowArr = arr[index].toLowerCase();
            repeatCount += (lowKey === lowArr) ? 1 : 0;
            continue;
        }

        repeatCount += (key === arr[index]) ? 1 : 0
    }
    value.push(key);
    countt.push(repeatCount)
  });

  console.log(value, countt)
...
let house = (data) => {
    return data.reduce((previousItem, currentItem) => {
      if (currentItem.price > previousItem.price) {
          return currentItem}
          return previousItem
    });
}

let filterHouse = (data) => {
...
let filterArea = (data, minPrice, maxPrice, minArea, maxArea) => {
    let filteredArea = data.filter(item => {
        return item.price >= minPrice && item.price <= maxPrice 
        && item.area >= minArea && item.area <= maxArea
    });

    (filteredArea.length === 0) ? console.log('Такого объекта нет.') : console.log(filteredArea)
}

filterArea(data, 150, 982, 10, 55868)
